...
> **Purpose**: Technical deep-dive. Master every class, relationship, design pattern, and trade-off so you can understand this system from scratch.
...
7. [Technical Best Practices & Common Questions](#7-technical-best-practices--common-questions)
...
|-------|-------|-----------------|
| **Customer** | [Customer.java](Customer.java) | Places orders, requests returns, receives notifications |
| **Delivery Person** | [DeliveryPerson.java](DeliveryPerson.java) | Delivers packages to lockers, picks up returns |
| **Locker Service (System)** | [LockerService.java](LockerService.java) | Manages all locations, finds lockers, verifies OTPs |
...
### 5.1 `LockerSize.java` — [View Source](LockerSize.java)
...
### 5.2 `LockerState.java` — [View Source](LockerState.java)
...
> The `CLOSED` state is defined but **not used** in the current codebase. This shows future extensibility — lockers can be taken offline for maintenance.

---

### 5.3 `Item.java` — [View Source](Item.java)
...
### 5.4 `Order.java` — [View Source](Order.java)
...
### 5.5 `Package.java` — [View Source](Package.java)
...
### 5.6 `LockerPackage.java` — [View Source](LockerPackage.java)
...
> **Technical insight**: If the code expires, what happens to the package? In real Amazon, it gets **returned to the fulfillment center**. This codebase doesn't implement that automatic return, which is a good discussion point about improvements.

---

### 5.7 `Locker.java` — [View Source](Locker.java)
...
### 5.8 `LockerLocation.java` — [View Source](LockerLocation.java)
...
### 5.9 `LockerService.java` — [View Source](LockerService.java)
...
### 5.10 `Customer.java` — [View Source](Customer.java)
...
### 5.11 `DeliveryPerson.java` — [View Source](DeliveryPerson.java)
...
### 5.12 `Notification.java` — [View Source](Notification.java)
...
### 5.13 `Driver.java` — [View Source](Driver.java)
...
The [Driver.java](Driver.java) demonstrates 3 scenarios. Here's exactly what happens at each step:
...
### 🎯 How to Present This Technical Design

**Step 1 — Gather requirements**: Define the scope. "Should I design for returns? Multiple locations? OTP expiry?"

**Step 2 — Identify actors & use cases**: Customer, DeliveryPerson, System. Use cases: place order, deliver, pickup, return.

**Step 3 — Core classes**: Define the class diagram. Start with enums (`LockerSize`, `LockerState`), then data classes (`Item`, `Order`, `Package`), then core classes (`Locker`, `LockerPackage`, `LockerLocation`), then the service (`LockerService`).

**Step 4 — Key methods**: Walk through `addPackage()`, `removePackage()`, `verifyCode()`, `requestLocker()`.

**Step 5 — Design patterns**: Singleton for `LockerService`, Inheritance for `Package→LockerPackage`, State Machine in `Locker`.
...
These are things you can mention to showcase technical depth:
...
### System Design Extension (scaling up)
...
> [!TIP]
> **Final advice**: In technical discussions, don't just write code. **Narrate your thinking**: "I'm using a Singleton here because..." "This enum prevents invalid states at compile time..." "I could improve this by..." It is important to show your **thought process**, not just the final answer.
